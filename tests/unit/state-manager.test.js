...
describe('State Manager', () => {
  ...
  beforeEach(() => {
    ...
    dom = new JSDOM(`
      ...
      <html>
        <body>
          ...
          <svg>
            <path id="riskGaugeArc" stroke="#f59e0b" stroke-dashoffset="200"></path>
          </svg>
        </body>
      </html>
    `, { url: 'http://localhost' });
    ...
    const stateManagerCode = require('fs').readFileSync('./state-manager.js', 'utf-8');
  ...
  describe('resetDashboard', () => {
    ...
    it('should reset label elements to N/A or Waiting', () => {
      ...
      expect(document.getElementById('sslHealthStatus').textContent).toBe('N/A');
      expect(document.getElementById('lastScanTime').textContent).toBe('Waiting');
  ...
  describe('enterResultState', () => {
    ...
    it('should transform warnings array to count', () => {
      const scanData = {
        network: { openPortsCount: 5, responseTimeMs: 150 },
        web: { sslStatus: 'valid' },
        issues: { criticalCount: 2, warnings: ['Warning 1', 'Warning 2', 'Warning 3'] }
      };
