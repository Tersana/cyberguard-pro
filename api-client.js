...
class APIClient {
  ...
  _buildURL(endpoint) {
    ...
    if (!fullURL.startsWith('https://')) {
      console.error('[APIClient] Security Error: All API requests must use HTTPS');
  ...
  async _handleResponse(response) {
    ...
    if (response.status === 401) {
      ...
      const isLoginEndpoint = response.url && response.url.includes('auth/login');
      if (!isLoginEndpoint) {
    ...
    if (response.status === 422) {
      ...
      const errors = this._transformValidationErrors(errorData.errors || errorData);
    ...
    if (response.status >= 500) {
      const error = new APIError('Server error. Please try again later.', response.status);
  ...
  async _request(method, endpoint, data = null, options = {}) {
    try {
      ...
      if (error instanceof TypeError && error.message.includes('fetch')) {
        const networkError = new NetworkError('Network error. Please check your connection and try again.');
